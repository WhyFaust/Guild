...
void ShowMenuModule(int iClient)
{
	...
	Format(sTitle, sizeof(sTitle), "%T [%i/%i]", "rpg_booster", iClient, g_iPerkLvl[iClient], g_Item.MaxLvl);
...
void KFG_load()
{
	...
	BuildPath(Path_SM, path, sizeof(path), "configs/gangs/gangs_rpg_booster.ini");
	if(!kfg.ImportFromFile(path)) SetFailState("[GANGS MODULE][RPG Booster] - Configuration file not found");
	kfg.Rewind();
