...
void ShowMenuModule(int iClient)
{
	...
	Format(sTitle, sizeof(sTitle), "%T [%i/%i]", "damage", iClient, g_iPerkLvl[iClient], g_Item.MaxLvl);
	...
	{
		Format(sItem, sizeof(sItem), "%t", "sell");
...
{
	if (IsValidClient(attacker) && IsValidClient(victim) && Gangs_ClientHasGang(attacker) && attacker != victim)
	{
...
void KFG_load()
{
	...
	if(!kfg.ImportFromFile(path)) SetFailState("[GANGS MODULE][Damage] - Файл конфигураций не найден");
	kfg.Rewind();
